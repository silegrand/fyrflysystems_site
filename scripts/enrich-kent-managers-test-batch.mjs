// scripts/enrich-kent-managers-test-batch.mjs
//
// TEST BATCH: pulls 20 Kent schools with a website and no business_manager_name
// yet, crawls each for a staff/leadership page using the tuned logic proven
// on the independent schools bursar run (ranked candidates, wide hint list,
// guessed-path fallback, multi-page concatenation), and asks Claude to find
// either a School Business Manager (or equivalent — Business Manager,
// Operations Manager, Office Manager with finance/ops remit) OR a
// Site/Facilities Manager (Site Manager, Premises Manager, Caretaker with
// a managerial title) — whichever is more clearly named on the site.
// Logs per-school outcome and a summary so we can check hit rate before
// scaling to the full ~540 schools with a website.
...
async function processSchool(school) {
  ...
  result.manager_email = extraction.email;
  return result;
}
...
async function main() {
  ...
  console.log(`\nFound a manager contact for ${counts.found || 0} / ${results.length} schools in this test batch.`);
}
