// scripts/enrich-south-east-state-managers-batch1-full.mjs
//
// Full run against all South East state schools (Surrey, East Sussex,
// West Sussex, Essex, Brighton & Hove) with a website and no
// business_manager_name yet. Same tuned crawl/extraction logic proven on
// independent schools (bursar), Kent schools, and London schools: ranked
// candidate pages, wide hint list, guessed-path fallback, multi-page
// concatenation. Looks for a School Business Manager (or equivalent) or,
// failing that, a Site/Facilities Manager.
//
// 1,361 schools — this will take a while sequentially (London's 185 took
// ~18 minutes, Kent's 534 took ~46 minutes, so expect roughly 2 hours).
// Safe to re-run: only pulls rows where business_manager_name is still
// null and skips ones already marked checked_not_found.
