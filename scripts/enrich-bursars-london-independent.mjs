// scripts/enrich-bursars-london-independent.mjs
//
// Bursar enrichment for the 425 London independent schools added to the
// independent_schools table. Same tuned crawl logic as the South East
// bursar run (v2): ranked candidate pages, wide hint list, guessed-path
// fallback, multi-page concatenation.
//
// Targets rows where region='London' and bursar_name is null.
// Paginates to avoid the 1000-row Supabase cap. Safe to re-run.
...
${pageText}`;
  ...
  try {
    ...
    return { found: false, name: null, title: null, email: null };
  }
}
...
async function main() {
  ...
  console.log(`London independent schools to enrich: ${schools.length}\n`);
  ...
  console.log(counts);
  console.log(`\nFound bursar for ${counts.found || 0} / ${results.length} London independent schools.`);
}
