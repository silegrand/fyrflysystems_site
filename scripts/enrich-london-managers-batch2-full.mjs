// scripts/enrich-london-managers-batch2-full.mjs
//
// Enrichment pass for all Greater London schools that have a website but
// no business_manager_name yet — covers the ~2,056 new records from the
// full 33-borough load plus any inner borough schools missed in batch 1.
//
// Same tuned crawl logic proven across Kent, London batch 1, and South East:
// ranked candidate pages, wide hint list, guessed-path fallback, multi-page
// concatenation. Looks for School Business Manager (preferred) or Site/
// Facilities Manager as fallback.
//
// Paginates the Supabase fetch to avoid the 1000-row cap (lesson learned
// from the South East run). Safe to re-run.
...
Find ONE of the following two roles, whichever is more clearly named on the page (prefer the Business Manager role if both are present):
...
2. Site Manager, or equivalent — Site/Facilities Manager, Premises Manager. Only use a Caretaker title if it is explicitly a "Site Manager" or "Head Caretaker" style managerial title.
...
If you cannot find either role with reasonable confidence, respond with found: false. Do not guess or invent a name.
...
${pageText}`;
  ...
  try {
    ...
    return { found: false, name: null, title: null, role_type: null, email: null };
  }
}
...
async function processSchool(school) {
  ...
  const candidateUrls = ranked.length > 0
    ...
    : GUESSED_PATHS.map((p) => {
        try { return new URL(p, school.website_url).toString(); } catch { return null; }
      }).filter(Boolean).slice(0, MAX_CANDIDATE_PAGES);

  const textChunks = [];
  for (const url of candidateUrls) {
    const html = await fetchText(url);
    if (html) textChunks.push(`--- Page: ${url} ---\n${stripHtml(html)}`);
  }
...
async function fetchAllUnprocessed() {
...
async function main() {
  const schools = await fetchAllUnprocessed();
  ...
  console.log(counts);
  const byRole = {};
  results.forEach((r) => { if (r.role_type) byRole[r.role_type] = (byRole[r.role_type] || 0) + 1; });
  console.log("By role type:", byRole);
  console.log(`\nFound a manager contact for ${counts.found || 0} / ${results.length} schools.`);
}
